...
class userRepository {
    ...
    */
     ...
     */
    public async updateScore(id: number, score: number): Promise<{ status: boolean }> {
        try {
            const sql = "UPDATE user SET score = ? WHERE id = ?";
            await this.db.query(sql, [score, id]);
    ...
    */
    public async deleteUserById(id: number): Promise<{ status: boolean }> {
        try {
